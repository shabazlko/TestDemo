import Foundation
import UIKit
class PopularModel: NSObject,Codable {
    let idKey = "id"
    let titleKey = "title"
    ...
    class func fromDictionary(_ array:NSArray) -> [PopularModel] {
        var returnArray:[PopularModel] = []
        for item in array {
            returnArray.append(PopularModel(dictionary: item as! NSDictionary))
        }
        return returnArray
    }
